...
def PLZTreinaAvaliaModelo(spark, classifier, features, classes, train, test, bucket, nome_bucket, ambiente_execucao_EMR):
    ...
    # Registra em log
    plz_grava_log( "Classificador: " + Mtype + " / Acuracia: " + str(accuracy), bucket)
    ...
    plz_upload_modelos_ml_bucket(fitModel, path , s3_path , bucket, ambiente_execucao_EMR)
    return result

# Função para criar o modelo de Machine Learning
def plz_cria_modelos_ml(spark, HTFfeaturizedData, TFIDFfeaturizedData, W2VfeaturizedData, bucket, nome_bucket, ambiente_execucao_EMR):
    ...
    for featureDF in featureDF_list:
        ...
        # Registra em log
        plz_grava_log( featureDF.name + " Resultados: ", bucket)
