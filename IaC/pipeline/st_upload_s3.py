...
def plz_upload_dados_processados_bucket(df, path, s3_path, bucket, ambiente_execucao_EMR):
    ...
    if ambiente_execucao_EMR:
        ...
        # Grava no log
        plz_grava_log("Log plz - Este Script Executa Somente em Cluster EMR", bucket)

# Define uma função para carregar um modelo de machine learning para um bucket S3
def plz_upload_modelos_ml_bucket(model, path, s3_path, bucket, ambiente_execucao_EMR):
