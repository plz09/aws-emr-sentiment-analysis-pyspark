...
def plz_limpa_transforma_dados(spark, bucket, nome_bucket, ambiente_execucao_EMR):
	...
	# Grava no log
	plz_grava_log("Log plz - Importando os dados...", bucket)

	# Carrega o arquivo CSV
	reviews = spark.read.csv(path+'dataset.csv', header=True, escape="\"")

	# Grava no log
	plz_grava_log("Log plz - Dados Importados com Sucesso.", bucket)
	plz_grava_log("Log plz - Total de Registros: " + str(reviews.count()), bucket)
	plz_grava_log("Log plz - Verificando se Existem Dados Nulos.", bucket)

	# Calcula os valores ausentes
	null_columns_calc_list = plz_calcula_valores_nulos(reviews)

	# Ação com base nos valores ausentes
	if (len(null_columns_calc_list) > 0):
		for column in null_columns_calc_list:
			plz_grava_log("Coluna " + str(column[0]) + " possui " + str(column[2]) + " de dados nulos", bucket)
		reviews = reviews.dropna()
		plz_grava_log("Dados nulos excluídos", bucket)
		plz_grava_log("Log plz - Total de Registros Depois da Limpeza: " + str(reviews.count()), bucket)
	else:
		plz_grava_log("Log plz - Valores Ausentes Nao Foram Detectados.", bucket)

	# Grava no log
	plz_grava_log("Log plz - Verificando o Balanceamento de Classes.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Existem " + str(count_positive_sentiment) + " reviews positivos e " + str(count_negative_sentiment) + " reviews negativos", bucket)

	# Cria o dataframe
	df = reviews

	# Grava no log
	plz_grava_log("Log plz - Transformando os Dados", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Limpeza dos Dados", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Os Dados de Texto Foram Limpos", bucket)
	plz_grava_log("Log plz - Tokenizando os Dados de Texto.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Removendo Stop Words.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Aplicando HashingTF.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Aplicando IDF.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Aplicando Word2Vec.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Padronizando os Dados com MinMaxScaler.", bucket)
	...
	# Grava no log
	plz_grava_log("Log plz - Salvando os Dados Limpos e Transformados.", bucket)
	...
	plz_upload_dados_processados_bucket(HTFfeaturizedData, path + 'HTFfeaturizedData', s3_path + 'HTFfeaturizedData' , bucket, ambiente_execucao_EMR)
	plz_upload_dados_processados_bucket(TFIDFfeaturizedData, path + 'TFIDFfeaturizedData', s3_path + 'TFIDFfeaturizedData', bucket, ambiente_execucao_EMR)
	plz_upload_dados_processados_bucket(W2VfeaturizedData, path + 'W2VfeaturizedData', s3_path + 'W2VfeaturizedData', bucket, ambiente_execucao_EMR)
